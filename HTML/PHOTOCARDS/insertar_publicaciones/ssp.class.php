...
static function order($request, $columns)
{
    ...
    // 👇 ORDEN POR DEFECTO SI NO VIENE NINGUNO
    if (!isset($request['order']) || !count($request['order'])) {
        return "ORDER BY precioCarta ASC";
    }
     ...
     */
    static function simple($request, $conn, $table, $primaryKey, $columns, $whereCustom = '', $searchFilter = array(), $searchFilter2 = array())
    {

        $whereCustom = ($whereCustom ? $whereCustom . ' AND ' : '') . 'cantidadCarta > 0';
        ...
        if (!empty($searchFilter2['filter2'])) {
            $sqlWhere = (strpos($where, 'WHERE') !== false) ? " AND " : " WHERE ";
            $whereAd = '';
            $i = 0;
            foreach ($searchFilter2['filter2'] as $key => $val) {
                $pre = ($i > 0) ? " AND " : "";
                $whereAd .= $pre . $key . " = '" . $val . "'";
                $i++;
            }
